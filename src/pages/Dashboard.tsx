...
function DelayedList({ vms, showAdmin, profiles }: { vms: TrackVM[]; showAdmin?: boolean; profiles?: Profile[] }) {
  ...
  if (!delayed.length) {
    return <Empty icon={<CheckCircle2 size={30} />} title="Sin demoras" sub="Todos los tracks activos están dentro del tiempo esperado (menos de 5 días sin avance)." />
  }
  return (
    ...
    <div className="flex flex-col gap-4">
      ...
      {!isStaff && vms.some((v) => v.status === 'en_progreso') && (
        <div className="text-[11px] text-[var(--muted)] flex items-center gap-2 px-1">
          <span className="w-2 h-2 rounded-full inline-block" style={{ background: DELAY_META.yellow.fg }} /> 5+ días sin avance
          <span className="w-2 h-2 rounded-full inline-block ml-2" style={{ background: DELAY_META.orange.fg }} /> 10+ días
          <span className="w-2 h-2 rounded-full inline-block ml-2" style={{ background: DELAY_META.red.fg }} /> 15+ días
